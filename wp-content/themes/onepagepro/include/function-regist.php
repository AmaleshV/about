<?php 
	...
	*/
	...
	add_filter('body_class', 'onepagepro_body_class');
	if( !function_exists('onepagepro_body_class') ){
		function onepagepro_body_class( $classes ) {
			$classes[] = 'onepagepro-body';
			$classes[] = 'onepagepro-body-front';

			$layout = onepagepro_get_option('general', 'layout', 'full');
			if( $layout == 'boxed' ){
			 	$classes[] = 'onepagepro-boxed';

			 	$border = onepagepro_get_option('general', 'enable-boxed-border', 'disable');
			 	if( $border == 'enable' ){
			 		$classes[] = 'onepagepro-boxed-border';
			 	}
			}else{
				$classes[] = 'onepagepro-full';
			}
			...
			if( empty($post_option['body-background-type']) || $post_option['body-background-type'] == 'default' ){
				$background = onepagepro_get_option('general', 'background-type', 'color');
			 	if( $background == 'pattern' ){
			 		$classes[] = 'onepagepro-background-pattern';
			 	}
			}

			$sticky_menu = onepagepro_get_option('general', 'enable-main-navigation-sticky', 'enable');
			if( $sticky_menu == 'enable' ){
				$classes[] = ' onepagepro-with-sticky-navigation';
				
				$sticky_menu_logo = onepagepro_get_option('general', 'enable-logo-on-main-navigation-sticky', 'enable');
				if( $sticky_menu_logo == 'disable' ){
					$classes[] = ' onepagepro-sticky-navigation-no-logo';
				}
			}
	...
	add_action('after_setup_theme', 'onepagepro_theme_setup');
	if( !function_exists( 'onepagepro_theme_setup' ) ){
		function onepagepro_theme_setup(){
			
			// define textdomain for translation
			load_theme_textdomain('onepagepro', get_template_directory() . '/languages');
			...
			register_nav_menus(array(
				'main_menu' => esc_html__('Primary Menu', 'onepagepro'),
				'right_menu' => esc_html__('Secondary Menu', 'onepagepro'),
				'mobile_menu' => esc_html__('Mobile Menu', 'onepagepro'),
			));
			...
			$thumbnail_sizes = onepagepro_get_option('plugin', 'thumbnail-sizing');
	...
	add_filter( 'wp_insert_post_data', 'onepagepro_page_default_comments_off' );
	if( !function_exists('onepagepro_page_default_comments_off') ){
		function onepagepro_page_default_comments_off( $data ) {
	...
	// logo displaying
	if( !function_exists('onepagepro_get_logo') ){
		function onepagepro_get_logo($settings = array()){

			$extra_class  = (isset($settings['padding']) && $settings['padding'] === false)? '': ' onepagepro-item-pdlr';
			$extra_class .= empty($settings['wrapper-class'])? '': ' ' . $settings['wrapper-class'];

			$ret  = '<div class="onepagepro-logo ' . esc_attr($extra_class) . '">';
			$ret .= '<div class="onepagepro-logo-inner">';
		
			// print logo
			if( !empty($settings['mobile']) ){
				$logo_id = onepagepro_get_option('general', 'mobile-logo');
			} 
			if( empty($logo_id) ){
				$logo_id = onepagepro_get_option('general', 'logo');
			}
	...
	// set anchor color
	add_action('wp_enqueue_scripts', 'onepagepro_set_anchor_color', 11);
	if( !function_exists('onepagepro_set_anchor_color') ){
		function onepagepro_set_anchor_color(){
			$post_option = onepagepro_get_post_option(get_the_ID());
				...
				foreach( $post_option['bullet-anchor'] as $anchor ){
					if( !empty($anchor['title']) ){
						...
						if( !empty($anchor['anchor-color']) ){
							$anchor_css .= '.onepagepro-bullet-anchor[data-anchor-section="' . esc_attr($anchor_section) . '"] a:before{ background-color: ' . esc_html($anchor['anchor-color']) . '; }';
						}
						if( !empty($anchor['anchor-hover-color']) ){
							$anchor_css .= '.onepagepro-bullet-anchor[data-anchor-section="' . esc_attr($anchor_section) . '"] a:hover, ';
							$anchor_css .= '.onepagepro-bullet-anchor[data-anchor-section="' . esc_attr($anchor_section) . '"] a.current-menu-item{ border-color: ' . esc_html($anchor['anchor-hover-color']) . '; }';
							$anchor_css .= '.onepagepro-bullet-anchor[data-anchor-section="' . esc_attr($anchor_section) . '"] a:hover:before, ';
							$anchor_css .= '.onepagepro-bullet-anchor[data-anchor-section="' . esc_attr($anchor_section) . '"] a.current-menu-item:before{ background: ' . esc_html($anchor['anchor-hover-color']) . '; }';
						}
					}
				}
			}
			...
			if( !empty($anchor_css) ){
				wp_add_inline_style('onepagepro-style-core', $anchor_css);
			}
		}
	}

	// remove id from nav menu item
	add_filter('nav_menu_item_id', 'onepagepro_nav_menu_item_id', 10, 4);
	if( !function_exists('onepagepro_nav_menu_item_id') ){
		function onepagepro_nav_menu_item_id( $id, $item, $args, $depth ){
			return '';
		}
	}

	// add additional script
	add_action('wp_head', 'onepagepro_header_script', 99);
	if( !function_exists('onepagepro_header_script') ){
		function onepagepro_header_script(){
			$header_script = onepagepro_get_option('plugin', 'additional-head-script', '');
	...
	add_action('wp_footer', 'onepagepro_footer_script');
	if( !function_exists('onepagepro_footer_script') ){
		function onepagepro_footer_script(){
			$footer_script = onepagepro_get_option('plugin', 'additional-script', '');
			if( !empty($footer_script) ){
				wp_add_inline_script('onepagepro-script-core', $footer_script);
			}
